...
# Copyright (C) 2020-2024 The Altair ROM Project
#
...
def create_dependency_manifest(dependencies):
    ...
    if len(projects) > 0:
        os.system("repo sync --force-sync --no-tags --current-branch --no-clone-bundle -j2 %s" % " ".join(projects))
...
def fetch_device(device):
    ...
    if not project is None:
        ...
        os.system('repo sync --force-sync --no-tags --current-branch --no-clone-bundle -j2 %s' % device_dir)
